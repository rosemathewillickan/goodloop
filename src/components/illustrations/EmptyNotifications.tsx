...
export function EmptyNotificationsIllustration({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 160 120" fill="none" className={className} aria-hidden="true">
      ...
      />
      <path d="M72 78a8 8 0 0 0 16 0" stroke={OUTLINE} strokeWidth="3.5" strokeLinecap="round" />
      <circle cx="34" cy="36" r="4" fill="var(--color-sky-300)" stroke={OUTLINE} strokeWidth="1.4" />
      <circle cx="128" cy="30" r="5" fill="var(--color-berry-300)" stroke={OUTLINE} strokeWidth="1.4" />
      <circle cx="136" cy="50" r="3" fill="var(--color-accent-300)" stroke={OUTLINE} strokeWidth="1.4" />
    </svg>
  );
}
